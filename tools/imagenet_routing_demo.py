...
from fast_rcnn.test import im_detect, im_detect_with_routing
...
with open(os.path.join(cfg.CACHE_DIR, 'synset_words.txt')) as f:
    synsets = f.read().splitlines()
    synsets_mappings = {i.split()[0]: i.split()[1] for i in synsets}


CLASSES = tree['object_list']
NETS = {'vgg16': ('VGG16',
...
def demo(net, image_name):
    ...
    im_file = os.path.join('data', 'ImageNet', 'ILSVRC2015', 'Data', 'DET', 'val',  image_name)
    ...
    for cls_ind, cls in enumerate(CLASSES[1:]):
        if cls not in synsets_mappings: continue
        class_name = synsets_mappings[cls]
...
if __name__ == '__main__':
    ...
    prototxt = os.path.join('models', 'imagenet', NETS[args.demo_net][0],
                            'routing', 'test.prototxt')
